...
function loadResponses(db: ReturnType<typeof getDb>, threadId: string) {
  return (db
    ...
    .all(threadId) as ResponseRow[]).map((response) => ({
    ...response,
...
export async function GET(
  ...
  try {
    ...
    const philosophers = db
      ...
      .all(id);
    const responses = loadResponses(db, id);
    const synthesis = loadSynthesis(db, id);
    const followUp = thread.follow_up_to ? null : loadFollowUp(db, id);
